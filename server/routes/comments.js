...
router.post('/', (req, res) => {
  try {
    const { package_id, package_title, author_name, author_email, content, rating } = req.body;
    if (!author_name || !content) return res.status(400).json({ error: 'Nome e comentário obrigatórios' });
    const result = run('INSERT INTO comments (package_id, package_title, author_name, author_email, content, rating) VALUES (?, ?, ?, ?, ?, ?)',
      [package_id, package_title, author_name, author_email, content, rating || 5]);
    res.status(201).json({ id: result.lastInsertRowid, message: 'Enviado para moderação' });
  } catch (err) { res.status(500).json({ error: 'Erro' }); }
});
